...
public class MinZipCodeRangeFilter {
	...
	final static Logger logger = Logger.getLogger(com.ws.zipcode.range.challenge.MinZipCodeRangeFilter.class);
	// pattern for checking valid range structure with 5 digits for both lower and upper bounds
	 ...
	 * @param zipCodeRanges takes ZipCodeRange array input
	...
	public static String mergeZipCodeRanges(ZipCodeRange[] zipCodeRanges) {
		...
		for (ZipCodeRange currentRange : zipCodeRanges) {
			...
			// check if the current range overlaps with top, if yes, push, otherwise merge
	...
	private static void convertUserArgsInputIntoZipCodeRanges(
		...
		for (String userInput : userInputs) {
			...
			if (matcher.find()) {
				...
				throw new InvalidUserInputsException(
						"Invalid input: " + userInput + " ,please enter valid user input with format: "
								+ VALID_INPUT_FORMAT);
	...
	public static void main(String[] args) {
        // build an ZipCodeRange array which shares the same size as user inputs'
